...
const ServiceFilterForm = () => {
    ...
    //Funcion para obtener servicios reales de la API
    const fetchServices = async () => {
        try {
            // Obtener servicios reales del backend
            const realServices = await fetchAllServicesApi(authToken, {});
    ...
    const handleSearchChange =  (event) => {
        ...
        // Logica de filtrado : se filtra los servicios por titulo, descripcion o tipos de servicio
        const newFilteredServices = services.filter(service =>
            (service.title && service.title.toLowerCase().includes(value.toLowerCase())) ||
            (service.description && service.description.toLowerCase().includes(value.toLowerCase())) ||
            (service.TiposServicio && service.TiposServicio.toLowerCase().includes(value.toLowerCase()))
        );
