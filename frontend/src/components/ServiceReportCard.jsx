...
const ServiceReportCard = ({ servicio, rol }) => {
  ...
  const direccion = servicio.Direccion || "No disponible";
  const servicios = servicio.TiposServicioCita || "No especificado";
  const estado = servicio.EstadoDescripcion || "Desconocido";
  ...
  if (servicio.Fecha && servicio.Hora) {
    try {
      const baseDate = new Date(servicio.Fecha);
      const [hours, minutes, seconds] = servicio.Hora.split(":").map(Number);
